...
async function deletePost(postIds) {
  console.log("in delete");
    const response = await axios({
      method: "delete",
      url: "api2/API/v1.0/tweets/"+localStorage.getItem("userName")+"/delete/"+postIds,
      headers: {
...
export const getProfilePosts = createAsyncThunk(
  "/api2/API/v1.0/tweets/"+localStorage.getItem("userName"),
  async (thunkAPI) => {
   
    const response = await axios({
      method: "get",
      url: "/api2/API/v1.0/tweets/"+localStorage.getItem("userName"),
      headers: {
        "token": localStorage.getItem("token"),
